...
import type { BlacklistDate, TestArea, TestCategory } from '../../types.js';
    ...
    const info = getDb()
      .prepare(
        `INSERT INTO test_areas
          (category_id, name, daily_capacity, min_days, max_days, is_active)
          VALUES (?, ?, ?, ?, ?, ?)`,
      )
      .run(
        p.data.category_id,
...
adminRouter.post('/blacklist', (req, res, next) => {
  try {
    ...
    const info = getDb()
      .prepare(
        `INSERT INTO blacklist_dates (test_area_id, date, reason)
         VALUES (?, ?, ?)`,
      )
      .run(p.data.test_area_id, p.data.date, p.data.reason ?? null);
    const row = getDb()
